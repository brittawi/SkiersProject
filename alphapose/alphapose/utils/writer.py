    ...
    def update(self):
        ...
        while True:
            ...
            if orig_img is None:
                ...
                if self.save_video:
                    stream.release()
                output_file = f"alphapose-results_{self.video_name}.json"
                if self.opt.save_json:
                    write_json(final_result, self.opt.outputpath, form=self.opt.format, for_eval=self.opt.eval, outputfile=output_file)
                    print("Results have been written to " + self.opt.outputpath + output_file)
            ...
            if boxes is None or len(boxes) == 0:
                ...
                if self.opt.save_img or self.save_video or self.opt.vis:
                    if hm_data.size()[1] == 49:
                        from alphapose.alphapose.utils.vis import vis_frame_dense as vis_frame
                    elif self.opt.vis_fast:
                        from alphapose.alphapose.utils.vis import vis_frame_fast as vis_frame
                    else:
                        from alphapose.alphapose.utils.vis import vis_frame
