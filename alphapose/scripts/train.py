...
def validate(m, opt, heatmap_to_coord, criterion, batch_size=20):
    ...
    with torch.no_grad():
        for inps, crop_bboxes, bboxes, img_ids, scores, imghts, imgwds in tqdm(det_loader, dynamic_ncols=True):
            ...
            if cfg.LOSS.get('TYPE') == 'MSELoss':
                loss = 0.5 * criterion(output, output)  # Assuming labels are not available, so using output itself
                acc = calc_accuracy(output, output)
            elif cfg.LOSS.get('TYPE') == 'Combined':
                if output.size()[1] == 68:
                    face_hand_num = 42
                else:
                ...
                loss_body_foot = 0.5 * criterion[0](output_body_foot, output_body_foot)
                acc_body_foot = calc_accuracy(output_body_foot, output_body_foot)

                loss_face_hand = criterion[1](output_face_hand, output_face_hand, None)
                acc_face_hand = calc_integral_accuracy(output_face_hand, output_face_hand, None, output_3d=False, norm_type=norm_type)

                loss = loss_body_foot + loss_face_hand
                acc = acc_body_foot * (output_body_foot.shape[1] / output.shape[1]) + acc_face_hand * (output_face_hand.shape[1] / output.shape[1])
            else:
                loss = criterion(output, output, None)
                acc = calc_integral_accuracy(output, output, None, output_3d=False, norm_type=norm_type)
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (i + 1) % opt.snapshot == 0:
            ...
            with torch.no_grad():
                ...
                # val_loss, val_acc = validate(m, opt, heatmap_to_coord, criterion, batch_size=20)
